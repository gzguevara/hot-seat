...
const App: React.FC = () => {
  ...
  const handleTransfer = useCallback(async (targetCharacter: Character, summary?: string) => {
    // We explicitly update the global state with 'targetCharacter' because it contains
    // the ephemeral prompt injections (Transfer Context, Final Juror Protocol) generated by useGeminiLive.
    // If we simply found the char by ID from the existing state, we would miss these prompt updates in the UI/Debug view.
    setCharacters(prev => prev.map(c => 
        c.id === targetCharacter.id ? targetCharacter : c
    ));
